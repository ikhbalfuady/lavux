...
<template >
  ...
  <q-form @submit="submit" class="col-12 row animated fadeIn" >
    ...
    <lv-container v-if="!loading" class="col-12" height="30vh">
      ...
      <lv-select col="4" label="Type" v-model="dataModel.type" :options="select.type" />
      <lv-input col="4" label="Slug" v-model="dataModel.slug" />
      <lv-input col="4" label="Name" v-model="dataModel.name" />
      <lv-textarea col="12" label="Value" v-model="dataModel.value" />
      <lv-textarea col="12" label="Description" v-model="dataModel.description" />
      <lv-textarea col="12" label="Remarks" v-model="dataModel.remarks" />
...
export default defineComponent({
  ...
  setup (props) {
    ...
    const select = reactive({ // select sources
      type: Handler.toObjectSelect(['string','double','integer','array','boolean','date']),
