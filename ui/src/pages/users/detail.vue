  ...
  <div :class="`${(fromModal) ? '' : 'q-pb-xs'} fix-content-modal row root`" style="min-width:75vw" v-if="Meta.permission.read">
    ...
    <lv-header-page class="bg-white" split showBack :backTo="{name: Meta.module}" :preventBackTo="fromModal ? true : false" >
      ...
      <template v-slot:right >
        <lv-btn v-if="!loading" soft label="Action" icon="more_horiz" color="indigo" labelVisibility>
    ...
    <lv-container v-if="!loading" class="col-12 " height="220px">
      <lv-uploader class="col-12 col-sm-4 col-md-3 text-center" size="180px" avatar rouded
        :placeholder="dataModel.picture" display
      />
      <div class="col-12 col-sm-8 col-md-9 row">
        <lv-view-item optimize-display label="name" :display="dataModel.name" />
        <lv-view-item optimize-display label="username" :display="dataModel.username" />
        <lv-view-item optimize-display label="email" :display="dataModel.email" />
        <lv-view-item optimize-display label="Ban?" :display="dataModel.is_ban" />
      </div>
